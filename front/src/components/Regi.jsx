...
function Registration() {
  ...
  const registrationAction = async (event) => {
    ...
    if (submission.username.length < 4) {
      ...
      !(isNaN(submission.phoneNumber) === false) ||
      submission.phoneNumber.length != 10
    ) {
    ...
    if (submission.typeOfUser == "Regular") {
      ...
      fetch(
        `main/register_user?usrname=${submission.username}&password=${submission.password}&email=${submission.email}&phoneNum=${submission.phoneNumber}&name=${submission.firstName}&surname=${submission.lastName}`,
        {
          method: "POST",
        }
      ).then((res) => {
      ...
      //TODO fix route to one that exists
      fetch(
        `main/register_shelter?usrname=${submission.username}&password=${submission.password}&email=${submission.email}&phoneNum=${submission.phoneNumber}&shelterName=${submission.shelterName}`,
        {
          method: "POST",
        }
      ).then((res) => {
        ...
        <Form onSubmit={registrationAction}>
          <div className="registration-details">
            <div className="form-divider">
              ...
              <div className="form-floating">
                <label htmlFor="datum-nestanka"></label>
                <input
                  type="date"
                  className="form-control"
                  id="DOB"
